...
<<<<<<< .mine
    ...
    def onchange_partner_id(self, cr, uid, ids, part):
        ...
        if o:
            ...
            for line in o.order_line:
                r_v = self.pool.get('sale.order.line').product_id_change(cr, uid, [line.id],
                    res['value']['pricelist_id'], line.product_id.id, qty=line.product_uom_qty,
                    partner_id=part,
                    packaging=line.product_packaging.id, fiscal_position=True
                )
                self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, 'r_v:%s'%r_v)
                u_line.append((
                    1, line.id, 
                    {'product_uom_qty':1.0}
                ))
        res['value']['order_line'] = u_line
