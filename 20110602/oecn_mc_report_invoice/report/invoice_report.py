...
class invoice_report(report_sxw.rml_parse):
        ...
        for b in bank_ids:            
            for r in bank.browse(self.cr, self.uid, [b['id']], {}):
                ...
                if r.bank.code == bkname or r.bank.name == bkname:
                    res += [(r.bank.name,r.bank.bic or '',r.acc_number,r.name or self.user.company_id.currency_id.name)]
	...
	if self.localcontext['company'].name == 'MASTER CONCEPT SINGAPORE PTE LTD':
