...
class sale_order(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if context.has_key('only_update_p'):
            return super(sale_order, self).write(cr, uid,  ids, vals, context=context)  
            self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, 'start2')                            
        
        for o in self.browse(cr, uid, ids, context=context):
            runflag = False        
            if o.state=='draft': #and o.related_id and o.related_id.state=='draft':#此处出错，如果新的partner需要建副本，则不能完成
                if 'partner_id' in vals and o.related_id and o.related_id.state=='draft' :
                    #伙伴改变,
                    ...
                    if buy_partner and (not buy_partner.createpurchase):
                        ...
                        self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, 'start3') 
                        res = super(sale_order, self).write(cr, uid, o.id, vals, context=context)  
                        runflag = True
                        ...
                        if new_uid_l:
                            ...
                            self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, '更新订单主表')                        
                            ...
                            if order_line in vals:
                                ...
                                for o_line in o.order_line:
                                    ...
                                    self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, '更新订单明细')                        
                else : #if 'partner_id' in vals:
                    #伙伴没改变,
                    #更新订单主表
                    self.write(cr, uid,o.related_id.id, new_vals, context)
                    self.logger.notifyChannel('addons.'+self._name, netsvc.LOG_INFO, '更新订单主表')                        
